...
public class SocketMessageHost
{
	...
	private class AcceptHandler implements Runnable {
		...
		{
			while(true) {
				synchronized(SocketMessageHost.this) {
					if(!running) {
						return;
					}
				}
	...
	private HashMap<MessageType, MessageHandler> messageHandlers = new HashMap<MessageType, MessageHandler>();
	
	private MessageFactory messageFactory = new MessageFactory();
	private MessageTypeFactory messageTypeFactory = new MessageTypeFactory();
	
	public SocketMessageHost(int port) {
		this.port = port;
	}
